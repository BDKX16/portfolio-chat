...
const workExperience = [
  {
    ...
    skills: [".NET Core", "LINQ", "NHibernate", "Vue", "SQL Server", "REST", "SOAP"],
  },
  {
    id: 2,
...
const education = [
  {
    ...
    description: "Specialized coursework in programming and information systems development.",
  },
]
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="relative ml-4 pl-10 border-l-2 border-muted mb-16">
        {workExperience.map((job, index) => (
          <RevealAnimation key={job.id} direction="left" delay={index * 0.1} width="100%">
                  ...
                  {job.skills && (
                    <div className="flex flex-wrap gap-2">
                      {job.skills.map((skill) => (
                        <span key={skill} className="px-3 py-1 bg-muted text-secondary rounded-full text-sm">
                          {skill}
                        </span>
                      ))}
                    </div>
      ...
      <div className="relative ml-4 pl-10 border-l-2 border-muted mb-16">
        {education.map((edu, index) => (
          <RevealAnimation key={edu.id} direction="left" delay={index * 0.1} width="100%">
            <div className="mb-12 relative">
              {/* Timeline dot */}
      ...
      <div className="relative ml-4 pl-10 border-l-2 border-muted mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {certifications.map((cert, index) => (
            <RevealAnimation key={cert.id} direction="left" delay={index * 0.05} width="100%">
                ...
                <GlowCard>
                  <div className="bg-card p-4 rounded-xl text-card-foreground">
                    <div className="flex flex-col">
                      <h4 className="text-md font-bold text-primary">{cert.title}</h4>
